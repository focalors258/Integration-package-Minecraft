StartupEvents.registry('entity_type', event => {
    ...
    //注意  据点实体的相关代码不可使用forge的persistentData  因为已有大量已存储实体使用了kubejs的pdata
    //注意  据点实体的相关代码不可使用forge的persistentData  因为已有大量已存储实体使用了kubejs的pdata
    event.create('combat_area_repeat', "entityjs:geckolib_projectile")
        // .immuneTo()//可接受的方块
            ...
            if (entity.age % 20 == 0) {
                ...
                entitys111.forEach(entitys => {
                    ...
                    if (entitys.isLiving()) {
                        ...
                        //if (!areData(entitys, 'camp_entity')) base = 1.5
                        ...
                        if (entitys.getRootData().contains('elite')) {
                            ...
                            //  if (entitys.isLiving()) {
                        } else if (entitys.isMonster() || areData(entitys, 'camp_entity')) {
                ...
                weight_quantity -= guard_quantity
                ...
                if (entity.server) {
                    if (sample && camp < 0 && generate && Math.random() < Math.pow((max_quantity - weight_quantity) / max_quantity, 0.9)) {//生成机制
                       ...
                       // tell("index " + index2)
                        //
                        //tell("enemy_sample " + entity.getRootData().getString('enemy_sample'))


                        // 
                        // tell("denominator " + denominator)
                        //  tell("index2 " + sample[index2])

                        // tell(entity.getRootData().getString("enemy_sample"))
                        // console.log(sample)
                        ...
                        范围内玩家.forEach(e => {
                            ...
                            //  setDan(e, getDan(e) + 100 + level * 0.2)重复
                        ...
                        if (getData(entity, 'boolean', 'done')) {
                            ...
                            setData(entity, 'boolean', 'done', false)

                            setData(entity, 'boolean', 'done2', false)

                            // entity.getRootData().remove('done')//当被抢夺时删除已被占领标记
                        }


                    } else if (camp > 0 && generate && Math.random() < Math.pow((max_quantity - guard_quantity) / max_quantity, 0.9)) {//当据点属于村民时

                        let summoner = entity.level.createEntity('guardvillagers:guard')//生成警卫
                        ...
                        范围内玩家.forEach(e => {
                            ...
                            setDan(e, getDan(e) + 30 + level * 0.1)
                ...
                if (entity.server) {//为服务端时  在此处统一修改camp (服务端 客户端)
                    ...
                    */
                    ...
                    if ((camp + campAdd) > maxCamp) {
                        ...
                        // tell("g" + absCamp)
                        // tell("g" + camp + pm * Math.pow(8 * absCamp, 0.8))
                    ...
                    //   tell("e" + Math.pow(8 * campAdd, 0.8))

                    //   tell("b" + camp + Math.pow(8 * campAdd, 0.8))

                    //     
                    //     
                    //     tell("c" + camp)

                    //     tell("d" + getData(entity, 'double', 'camp'))
