NativeEvents.onEvent('com.integration_package_core.event.Event.ItemTransferEvent', e => {
    ...
    if (e.itemStack.hasTag('kubejs:combat_weapon')) {
        if (!item.nbt.contains("load")) {//初始化武器
            ...
            if (weaponList[item.id]["skill0"]) {

                //   if(weaponList[item.id]["skill0"]["type"])

                item.nbt.putInt("skill0_level", 1)
        ...
        // initPlayerToughnessList(e.player, item)//初始化玩家数据

        pushUseCombatServer(e.player, item.id, item.nbt)//添加武器到列表
        // e.player.pushWeapon(item)
